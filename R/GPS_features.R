...
extractFixTypePALMSOneFile = function(inputFile, outputDir, winSize, names=NULL){
  # function to extract the fix type from a PALMS file
  
  if (winSize != 60){
    stop("not implemented for window sizes other than 60s")
  }
  dat = read.csv(inputFile, header=TRUE, stringsAsFactors=FALSE, 
                 colClasses=c(identifier="character"))
  dat = data.frame(timestamp=strptime(dat$dateTime, format="%Y-%m-%d %H:%M:%S"),
                   identifier=dat$identifier, 
                   stationary=str_detect(dat$fixType, "stationary"), 
                   stringsAsFactors=FALSE)
  dat$timestamp = strftime(dat$timestamp, format="%Y-%m-%d %H:%M:00")
  ids = unique(dat$identifier)
  for (i in 1:length(ids)) {
    cat(ids[i], "\n")
    dat_id = dat[dat$id==ids[i],c("timestamp","stationary")]
    x = aggregate(dat_id$stationary, by=list(dat_id$timestamp), sum)
    y = aggregate(dat_id$stationary, by=list(dat_id$timestamp), length)
    dat_id = data.frame(timestamp = as.character(x$Group.1),
                        stationary=x$x/y$x, stringsAsFactors=FALSE)
    if (!file.exists(outputDir)) {
      dir.create(outputDir, recursive=TRUE)
    }
    savefile = file.path(outputDir, paste0(ids[i], ".csv"))
    write.csv(dat_id, file=savefile, quote=FALSE, row.names=FALSE)
  }
}
extractFixTypePALMSDirectory = function(inputDir, outputDir, winSize, names=NULL){
  # function to extract the fix type from a directory of PALMS files
  ...
  for (i in 1:length(files)){
    inputFile = file.path(inputDir, files[i])
    dat = read.csv(inputFile, header=TRUE, stringsAsFactors=FALSE, 
                   colClasses=c(identifier="character"))
    dat = data.frame(timestamp=strptime(dat$dateTime, format="%Y-%m-%d %H:%M:%S"),
                     stationary=str_detect(dat$fixType, "stationary"),
                     stringsAsFactors=FALSE)
    dat$timestamp = strftime(dat$timestamp, format="%Y-%m-%d %H:%M:00")
    x = aggregate(dat$stationary, by=list(dat$timestamp), sum)
    y = aggregate(dat$stationary, by=list(dat$timestamp), length)
    dat = data.frame(timestamp = as.character(x$Group.1),
                        stationary=x$x/y$x, stringsAsFactors=FALSE)
    if (!file.exists(outputDir)) {
      dir.create(outputDir, recursive=TRUE)
    }
    savefile = file.path(outputDir, files[i])
    write.csv(dat, file=savefile, quote=FALSE, row.names=FALSE)
  }
}
singlePlaceFeatures = function(input, winSize, names=NULL){
  # function to create single place features
  if (file.info(input)$isdir) {
    statDir = paste(input, "Stationary", as.character(winSize), sep="_")
    if (!file.exists(statDir)) {
      extractFixTypePALMSDirectory(input, statDir, winSize, names)
    }
  } else {
    statDir = paste(file_path_sans_ext(input), "Stationary", as.character(winSize), sep="_")
    if (!file.exists(statDir)) {
      extractFixTypePALMSOneFile(input, statDir, winSize)
    }
  }
  return(statDir)
}